...
/** Fits a legacy provider's native layout as a whole; retains RemoteViews and click actions. */
internal class FittedNativeWidget(context: Context, private val host: AppWidgetHostView,
    private val minimumHeightDp: Float) : FrameLayout(context) {
    init { addView(host); clipChildren = true }
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val width = View.MeasureSpec.getSize(widthMeasureSpec).coerceAtLeast(1)
        val height = View.MeasureSpec.getSize(heightMeasureSpec).coerceAtLeast(1)
        val fit = nativeWidgetFit(width, height, minimumHeightDp * resources.displayMetrics.density)
        host.pivotX = 0f; host.pivotY = 0f
        host.scaleX = fit.scale; host.scaleY = fit.scale
        host.measure(View.MeasureSpec.makeMeasureSpec(fit.width, View.MeasureSpec.EXACTLY),
            View.MeasureSpec.makeMeasureSpec(fit.height, View.MeasureSpec.EXACTLY))
