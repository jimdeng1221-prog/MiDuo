...
/** Dissolve in one undoable edit. Keep every child on Home; never touch installed apps. */
fun dissolveFolder(layout: HomeLayout, folderId: String): HomeLayout {
    val folder = layout.folder(folderId) ?: return layout
    val original = layout.indexOfShortcut(folderId)
    var next = layout.withoutShortcut(folderId).copy(folders = layout.folders.filterNot { it.id == folderId })
    val blocked = widgetCellsForFolders(next)
    folder.appIds.forEachIndexed { index, appId ->
        val target = if (index == 0 && original != null && original !in blocked) original else {
            val page = original?.let(::homeCellPage) ?: 0
            val nearby = (0 until HOME_CELLS).map { homeCellIndex(page, it) }
            nearby.firstOrNull { it !in blocked && next.slotAt(it) == null }
                ?: generateSequence(0) { it + 1 }.first { it !in blocked && next.slotAt(it) == null }
        }
        next = next.withSlot(target, appId)
    }
    return next
}
