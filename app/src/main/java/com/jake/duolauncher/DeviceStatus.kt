...
data class DeviceStatus(
    ...
    val cellularTechnology: CellularTechnology = CellularTechnology.UNKNOWN,
...
class DeviceStatusMonitor(private val context: Context) : DefaultLifecycleObserver {
    ...
    private val phoneCallback = object : TelephonyCallback(), TelephonyCallback.SignalStrengthsListener,
        ...
        override fun onDisplayInfoChanged(telephonyDisplayInfo: TelephonyDisplayInfo) {
            mutable.update { it.copy(cellularTechnology = cellularTechnology(
                telephonyDisplayInfo.networkType, telephonyDisplayInfo.overrideNetworkType)) }
        }
    }
    ...
    override fun onStart(owner: LifecycleOwner) {
        ...
        mutable.update { it.copy(
            cellularLevel = runCatching { phone.signalStrength?.level }.getOrNull(),
            // getDataNetworkType requires a phone-state permission that a launcher
            // should not request. The registered display-info callback fills this
            // value when the platform makes it available.
            cellularTechnology = CellularTechnology.UNKNOWN,
        ) }
        updateConnection()
    }
